/* PID.ino: Fan Control using a PID strategy
Author: David Dubins
Date: September 2018
Last Updated: 13-Mar-25
Connections:
+5V to LM35 Pin 1
Arduino Pin A0 to LM35 Pin 2
GND to LM35 Pin 3
...
void myPID(float kP, float kI, float kD) {
  ...
  */
  ...
  float I = 0.0;
  float D = 0.0;
  static float LAST=0.0; // static remembers LAST on exit
  long DRIVE = 0;      // to store DRIVE value
  MEASURED = analogRead(MESPin) * 500.0/1023.0; //convert div to degC
